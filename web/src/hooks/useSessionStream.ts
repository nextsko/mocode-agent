 ...
 *
 * 2) Browser tabs (two Mocode pages open in Chrome, etc.):
 *
...
import { mocodeVersion } from "@/lib/version";
...
export function useSessionStream(
  ...
  const replaySeqRef = useRef(0); // Deterministic counter for replay message IDs
  ...
  // During history replay, uses a deterministic sequence counter so that
  // the same event always maps to the same ID across reconnects — this
  // allows upsertMessage to deduplicate correctly.
  const getNextMessageId = useCallback(
    (prefix: "user" | "assistant"): string => {
      if (isReplayingRef.current) {
        const seq = replaySeqRef.current++;
        return `${prefix}-replay-${seq}`;
      }
      ...
      switch (event.type) {
        ...
        case "ToolResult": {
          ...
          if (Array.isArray(return_value.output)) {
            ...
            // For non-browser-renderable URLs (e.g. ms:// from model),

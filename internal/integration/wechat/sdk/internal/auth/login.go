...
func Login(ctx context.Context, client *protocol.Client, opts LoginOptions) (*Credentials, error) {
	...
	for {
		...
		if opts.OnQRURL != nil {
			...
			slog.Info("WeChat QR URL", "url", qr.QRCodeImgURL)
		}
		...
		for {
			...
			if status.Status != lastStatus {
				...
				case "scaned":
					if opts.OnScanned != nil {
						...
						slog.Info("WeChat QR scanned, confirm in WeChat")
					}
				case "expired":
					if opts.OnExpired != nil {
						opts.OnExpired()
					} else {
						slog.Debug("WeChat QR expired, requesting new one")
					}
				case "confirmed":
					slog.Info("WeChat login confirmed")
				}
			}
			...
			if status.Status == "confirmed" {
				...
				if err := SaveCredentials(creds, opts.CredPath); err != nil {
					slog.Warn("Could not save WeChat credentials", "error", err)
				}
				return creds, nil
			}
			...
			if status.Status == "scaned_but_redirect" {
				if status.RedirectHost != "" {
					...
					slog.Debug("WeChat IDC redirect", "host", status.RedirectHost)
				}

...
// UpdateMessage updates a workspace message.
func (c *Client) UpdateMessage(ctx context.Context, id string, msg proto.Message) error {
	rsp, err := c.put(ctx, fmt.Sprintf("/workspaces/%s/messages/%s", id, msg.ID), nil, jsonBody(msg), http.Header{"Content-Type": []string{"application/json"}})
	if err != nil {
		return fmt.Errorf("failed to update message: %w", err)
	}
	defer rsp.Body.Close()
	if rsp.StatusCode != http.StatusOK {
		return fmt.Errorf("failed to update message: status code %d", rsp.StatusCode)
	}
	return nil
}

// DeleteMessage deletes a workspace message.
func (c *Client) DeleteMessage(ctx context.Context, id string, messageID string) error {
	rsp, err := c.delete(ctx, fmt.Sprintf("/workspaces/%s/messages/%s", id, messageID), nil, nil)
	if err != nil {
		return fmt.Errorf("failed to delete message: %w", err)
	}
	defer rsp.Body.Close()
	if rsp.StatusCode != http.StatusOK {
		return fmt.Errorf("failed to delete message: status code %d", rsp.StatusCode)
	}
	return nil
}

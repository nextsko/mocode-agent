...
// On first run (or version mismatch), syncs built-in modes to ~/.skcode/agents/.
func (s *ConfigStore) SetupAgents() {
	dir := resolveAgentsDir(s.config)
	if err := SyncAgentsToDir(dir); err != nil {
		slog.Warn("Failed to sync agents to disk", "dir", dir, "error", err)
	}
	s.config.SetupAgents()
}

// ReloadAgent reloads a single agent from the agents directory.
// This is called by the AgentWatcher when a file change is detected.
func (s *ConfigStore) ReloadAgent(id string, path string) error {
	// Load the agent from file
	agent, err := loadAgentMDFile(path, id, s.config.Options.DisabledTools)
	if err != nil {
	...
	if agent.AllowedTools == nil {
		agent.AllowedTools = resolveAllowedTools(allToolNames(), s.config.Options.DisabledTools)
	} else {
		agent.AllowedTools = resolveAllowedTools(agent.AllowedTools, s.config.Options.DisabledTools)
	}

	// Update the agent in config
	s.config.Agents[id] = agent

	slog.Info("Agent reloaded successfully", "id", id, "name", agent.Name)
	return nil
}

// RemoveAgent removes an agent from the config.
// This is called by the AgentWatcher when a file is deleted.
func (s *ConfigStore) RemoveAgent(id string) {
	// Don't remove built-in agents
	if id == AgentCoder || id == AgentTask {
		return
	}

	delete(s.config.Agents, id)
	slog.Info("Agent removed", "id", id)
}

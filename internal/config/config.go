...
// AppName returns the executable name derived from os.Args[0].
func AppName() string {
	return filepath.Base(os.Args[0])
}

// GetAppName returns the display brand name from config, falling back to defaultAppName.
func GetAppName(cfg *Config) string {
	if cfg != nil && cfg.Options != nil && cfg.Options.AppName != "" {
		return cfg.Options.AppName
	}
...
type Attribution struct {
	...
	GeneratedWith bool         `json:"generated_with,omitempty" jsonschema:"description=Add Generated with mocode line to commit messages and issues and PRs,default=true"`
}
...
type Options struct {
	...
	SkillsPaths               []string        `json:"skills_paths,omitempty" jsonschema:"description=Paths to directories containing Agent Skills (folders with SKILL.md files),example=~/.config/mocode/skills,example=./skills"`
	TUI                       *TUIOptions     `json:"tui,omitempty" jsonschema:"description=Terminal user interface options"`
	Debug                     bool            `json:"debug,omitempty" jsonschema:"description=Enable debug logging,default=false"`
	DebugLSP                  bool            `json:"debug_lsp,omitempty" jsonschema:"description=Enable debug logging for LSP servers,default=false"`
	DisableAutoSummarize      bool            `json:"disable_auto_summarize,omitempty" jsonschema:"description=Disable automatic conversation summarization,default=false"`
	DataDirectory             string          `json:"data_directory,omitempty" jsonschema:"description=Directory for storing application data (relative to working directory),default=.mocode,example=.mocode"` // Relative to the cwd
...
// RecordType identifies which kind of debug record to capture.
type RecordType string

const (
	RecordTypeToolCalls      RecordType = "tool_calls"
	RecordTypeErrors         RecordType = "errors"
	RecordTypeMCPRequests    RecordType = "mcp_requests"
	RecordTypeMCPResponses   RecordType = "mcp_responses"
	RecordTypeLoopDetections RecordType = "loop_detections"
)

// RecordsOptions configures debug data recording.
...
type Agent struct {
	...
	// Tools to remove from the default set (applied after AllowedTools)
	...
	// SystemPrompt overrides the default system prompt for this agent.
	SystemPrompt string `json:"system_prompt,omitempty"`

	// SubAgents lists the IDs of modes this agent can delegate to via transfer.
	SubAgents []string `json:"sub_agents,omitempty"`

	// Temperature overrides the model temperature for this agent.
...
// Config holds the configuration for mocode.
type Config struct {
...
func resolveReadOnlyTools(tools []string) []string {
	readOnlyTools := []string{"glob", "grep", "ls", "read_files", "sourcegraph", "view", "crawl", "download_docs", "memory_search", "memory_load", "git_plan_commits"}
...
func (c *Config) SetupAgents() {
	...
	// Apply context paths and tool filters.
	for id := range agents {
		a := agents[id]
		if c.Options.ContextPaths != nil && a.ContextPaths == nil {
			a.ContextPaths = c.Options.ContextPaths
		}
		if a.AllowedTools == nil || id == AgentCoder {
			a.AllowedTools = resolveAllowedTools(allToolNames(), c.Options.DisabledTools)
		} else {
	...
	// Task agent gets read-only tools.
...
func (c *ProviderConfig) TestConnection(resolver VariableResolver, clients ...*http.Client) error {
	var (
	...
	case catwalk.TypeAnthropic:
		...
		case catwalk.InferenceKimiCoding:
			// Kimi provider (Anthropic-compatible API) uses /v1/models endpoint.

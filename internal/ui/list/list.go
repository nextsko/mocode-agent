...
type List struct {
	...
	// showScrollbar indicates whether to show a scrollbar when content overflows.
	showScrollbar bool

	// scrollbarWidth is the width of the scrollbar in characters.
	scrollbarWidth int

	// styles holds the styles for rendering the scrollbar.
...
// Render renders the list and returns the visible lines (without scrollbar).
// Use RenderScrollbar() to get the scrollbar separately.
...
// RenderScrollbar returns the scrollbar as a string if it should be shown.
// Returns empty string if no scrollbar is needed.
func (l *List) RenderScrollbar() string {
	if !l.showScrollbar || l.styles == nil || len(l.items) == 0 {
		return ""
	}
	...
	// Calculate current scroll offset in lines
	scrollOffset := 0
	...
	scrollOffset += l.offsetLine

	// Render scrollbar
	return common.Scrollbar(l.styles, l.height, totalHeight, l.height, scrollOffset)
}

// SetShowScrollbar enables or disables scrollbar rendering.
func (l *List) SetShowScrollbar(show bool) {
	l.showScrollbar = show
}

// SetScrollbarStyles sets the styles used for rendering the scrollbar.
func (l *List) SetScrollbarStyles(s *styles.Styles) {
	l.styles = s
}

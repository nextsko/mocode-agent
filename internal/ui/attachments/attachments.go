...
func (r *Renderer) Render(attachments []message.Attachment, deleting bool, width int) string {
	...
	// Show removal hint when there are attachments and not in delete mode.
	if len(attachments) > 0 && !deleting {
		hint := lipgloss.NewStyle().Foreground(lipgloss.Color("240")).Render(" ctrl+r remove")
		chips = append(chips, hint)
	}

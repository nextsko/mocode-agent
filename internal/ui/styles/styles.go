...
type Styles struct {
	...
	Header struct {
		...
		LogoGradCanvas    lipgloss.Style // Canvas for the compact "MOCODE" gradient
		LogoGradFromColor color.Color    // "MOCODE" wordmark gradient start
		LogoGradToColor   color.Color    // "MOCODE" wordmark gradient end
	}
	...
	Logo struct {
		...
		SmallGradFromColor color.Color    // Small "Mocode" wordmark gradient start
		SmallGradToColor   color.Color    // Small "Mocode" wordmark gradient end
	}
	...
	// Toast notification styles (floating bottom-right overlay).
	Toast struct {
		ErrorIndicator   lipgloss.Style
		WarnIndicator    lipgloss.Style
		InfoIndicator    lipgloss.Style
...
// QuitSummary holds styles for the quit summary output.
type QuitSummary struct {
	Frame             lipgloss.Style // Outer rounded border framing the quit summary output
	Title             lipgloss.Style // Title badge ("mocode session summary") with background
	SectionHeader     lipgloss.Style // Section divider header text ("── Session ──")
	Label             lipgloss.Style // Key label (bold, e.g. "status", "duration")
	Value             lipgloss.Style // Value text (e.g. "completed", "3s")
	StatusCompleted   lipgloss.Style // Completed status indicator (green)
	StatusInterrupted lipgloss.Style // Interrupted status indicator (red)
	Divider           lipgloss.Style // Horizontal separator line between sections
	Resume            lipgloss.Style // Resume command line (gold)
}

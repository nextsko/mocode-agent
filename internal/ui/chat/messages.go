...
// Set MOCODE_RENDER_CACHE_DEBUG=1 to enable.
var renderCacheDebug = os.Getenv("MOCODE_RENDER_CACHE_DEBUG") == "1"

// cacheStats tracks render cache statistics.
...
// getCachedRenderWithLog returns the cached render with debug logging.
func (c *cachedMessageItem) getCachedRenderWithLog(width int, itemType string) (string, int, bool) {
	if c.width == width && c.rendered != "" {
		if renderCacheDebug {
			cacheStats.hits.Add(1)
			slog.Debug("render cache hit", "type", itemType, "width", width, "height", c.height)
		}
		return c.rendered, c.height, true
	}
	if renderCacheDebug {
		...
		reason := "empty"
		if c.rendered != "" {
			reason = fmt.Sprintf("width_mismatch(cached=%d,requested=%d)", c.width, width)
		}
		slog.Debug("render cache miss", "type", itemType, "width", width, "reason", reason)
	}
	return "", 0, false
}
...
func (a *AssistantInfoItem) RawRender(width int) string {
	...
	content, _, ok := a.getCachedRenderWithLog(innerWidth, "assistant_info")
	if !ok {
...
func (a *AssistantInfoItem) renderContent(width int) string {
	...
	if model == nil {
		...
		model = &catwalk.Model{Name: modelName}
	}
...
func cappedMessageWidth(availableWidth int) int {
	return max(min(availableWidth-MessageLeftPaddingTotal, maxTextWidth), 20)
}

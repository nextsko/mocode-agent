...
func (a *AssistantMessageItem) RawRender(width int) string {
	...
	content, height, ok := a.getCachedRenderWithLog(cappedWidth, "assistant")
	if !ok {
...
func (a *AssistantMessageItem) renderThinking(thinking string, width int) string {
	...
	if renderer == nil {
		...
		r, err := renderer.Render(thinking)
		if err != nil {
			rendered = thinking
		} else {
			rendered = r
...
func (a *AssistantMessageItem) renderMarkdown(content string, width int) string {
	...
	if renderer == nil {
		// DEBUG: Log nil renderer
		fmt.Printf("[DEBUG] Nil markdown renderer for width: %d\n", width)
		return content
	}
	result, err := renderer.Render(content)
	if err != nil {
		// DEBUG: Log markdown render errors
		fmt.Printf("[DEBUG] Markdown render error: %v\nWidth: %d\nContent preview: %.100s\n", err, width, content)
		return content
	}

...
// ReadFilesToolMessageItem is a message item for read_files.
type ReadFilesToolMessageItem struct {
	*baseToolMessageItem
}

var _ ToolMessageItem = (*ReadFilesToolMessageItem)(nil)

// NewReadFilesToolMessageItem creates a new [ReadFilesToolMessageItem].
func NewReadFilesToolMessageItem(
	sty *styles.Styles,
	toolCall message.ToolCall,
	result *message.ToolResult,
	canceled bool,
) ToolMessageItem {
	return newBaseToolMessageItem(sty, toolCall, result, &ReadFilesToolRenderContext{}, canceled)
}

// ReadFilesToolRenderContext renders read_files tool messages.
type ReadFilesToolRenderContext struct{}
...
func (r *ReadFilesToolRenderContext) RenderTool(sty *styles.Styles, width int, opts *ToolRenderOpts) string {
	...
	header := toolHeader(sty, opts.Status, "Read Files", cappedWidth, opts.Compact, toolParams...)
	if opts.Compact {
		return header
	}

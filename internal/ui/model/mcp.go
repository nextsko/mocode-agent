...
func (m *UI) mcpSummaryCounts() (servers, enabled, tools, prompts, resources int) {
	cfg := m.com.Config()
	if cfg == nil {
		return 0, 0, 0, 0, 0
	}
	for _, item := range cfg.MCP.Sorted() {
		...
		state, ok := m.mcpStates[item.Name]
		if !ok {
			continue
		}
		if state.State != mcp.StateDisabled && item.MCP.Disabled {
			enabled++
		}
		tools += state.Counts.Tools
		prompts += state.Counts.Prompts
		resources += state.Counts.Resources
	}
	return servers, enabled, tools, prompts, resources
}

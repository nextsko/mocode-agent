...
import (
	...
	"github.com/package-register/mocode/internal/agent/tools/plugins/searchcommon"
...
func TestGrepWithIgnoreFiles(t *testing.T) {
	...
	// Create .mocodeignore file
	mocodeignoreContent := "node_modules/\n"
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, ".mocodeignore"), []byte(mocodeignoreContent), 0o644))
	...
	for name, fn := range map[string]func(pattern, path, include string) ([]GrepMatch, error){
		...
		t.Run(name, func(t *testing.T) {
			...
			require.False(t, foundFiles["lib.js"], "Should not find lib.js (ignored by .mocodeignore)")
...
func TestSearchImplementations(t *testing.T) {
	...
	require.NoError(t, os.WriteFile(filepath.Join(tempDir, ".mocodeignore"), []byte("file5.txt\n"), 0o644))

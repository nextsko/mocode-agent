...
func createTestLogFile(t *testing.T, entries []map[string]any) string {
	t.Helper()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "Mocode.log")
...
func TestNewMocodeLogsTool(t *testing.T) {
	t.Parallel()
	tool := NewMocodeLogsTool("/tmp/test.log")
	require.NotNil(t, tool)
	require.Equal(t, MocodeLogsToolName, tool.Info().Name)
}

func TestMocodeLogs_HappyPath(t *testing.T) {
	t.Parallel()
	...
	result := runMocodeLogs(logFile, MocodeLogsParams{Lines: 3})
...
func TestMocodeLogs_DefaultLines(t *testing.T) {
	t.Parallel()
	...
	result := runMocodeLogs(logFile, MocodeLogsParams{Lines: 0})
...
func TestMocodeLogs_MaxCap(t *testing.T) {
	t.Parallel()
	...
	result := runMocodeLogs(logFile, MocodeLogsParams{Lines: 200})

	lines := strings.Split(result, "\n")
	require.Len(t, lines, 100)
}

func TestMocodeLogs_MissingFile(t *testing.T) {
	t.Parallel()
	result := runMocodeLogs("/nonexistent/path/Mocode.log", MocodeLogsParams{Lines: 50})
	require.Contains(t, result, "No log file found")
}

func TestMocodeLogs_EmptyFile(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "Mocode.log")
	...
	result := runMocodeLogs(logFile, MocodeLogsParams{Lines: 50})
	require.Contains(t, result, "Log file is empty")
}

func TestMocodeLogs_MalformedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "Mocode.log")
	...
	result := runMocodeLogs(logFile, MocodeLogsParams{Lines: 10})
...
func TestMocodeLogs_ExtraFieldsSorted(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runMocodeLogs(logFile, MocodeLogsParams{Lines: 1})
...
func TestMocodeLogs_NonStringValues(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestMocodeLogs_Redaction(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestMocodeLogs_ReservedFields(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
		{
			"time":   time.Now().Format(time.RFC3339),
...
func TestMocodeLogs_OversizedLines(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "Mocode.log")
...
func TestMocodeLogs_PartialTrailingLine(t *testing.T) {
	t.Parallel()
	tempDir := t.TempDir()
	logFile := filepath.Join(tempDir, "Mocode.log")
...
func TestMocodeLogs_ValueQuoting(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
...
func TestMocodeLogs_ChronologicalOrder(t *testing.T) {
	t.Parallel()
...
func TestMocodeLogs_TimeOnlyFormat(t *testing.T) {
	t.Parallel()
	entry := map[string]any{
...
func TestMocodeLogs_LevelVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{
	...
	result := runMocodeLogs(logFile, MocodeLogsParams{Lines: 5})
...
func TestMocodeLogs_SourceVariations(t *testing.T) {
	t.Parallel()
	entries := []map[string]any{

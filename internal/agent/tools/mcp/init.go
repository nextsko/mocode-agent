...
// clients within the Mocode application.
package mcp
...
func initClient(ctx context.Context, cfg *config.ConfigStore, name string, m config.MCPConfig, resolver config.VariableResolver) error {
	...
	session, err := createSession(ctx, name, m, cfg)
	if err != nil {
		return err
	}
...
func getOrRenewClient(ctx context.Context, cfg *config.ConfigStore, name string) (*ClientSession, error) {
	...
	sess, err = createSession(ctx, name, m, cfg)
	if err != nil {
		return nil, err
	}
...
func createTransport(ctx context.Context, m config.MCPConfig, cfg *config.ConfigStore) (mcp.Transport, error) {
	...
	case config.MCPHttp:
		...
		client := &http.Client{
			Transport: &headerRoundTripper{
				...
				next:    cfg.Config().HTTPTransport(resolver),
			},
		}

...
func TestReadBuiltinFile(t *testing.T) {
	...
	t.Run("reads mocode-config skill", func(t *testing.T) {
		t.Parallel()

		resp, err := readBuiltinFile(ViewParams{
			FilePath: "mocode://skills/mocode-config/SKILL.md",
		}, nil)
		require.NoError(t, err)
		require.NotEmpty(t, resp.Content)
		require.Contains(t, resp.Content, "Mocode Configuration")
	})
	...
	t.Run("not found", func(t *testing.T) {
		...
		resp, err := readBuiltinFile(ViewParams{
			FilePath: "mocode://skills/nonexistent/SKILL.md",
		}, nil)
		require.NoError(t, err)
	...
	t.Run("metadata has skill info", func(t *testing.T) {
		...
		require.Equal(t, "mocode-config", meta.ResourceName)

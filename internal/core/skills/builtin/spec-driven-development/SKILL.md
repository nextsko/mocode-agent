...
description: >-
  ...
  significant change and no specification exists yet. Use when requirements
  are unclear, ambiguous, or only exist as a vague idea. For the execution
  and progress-tracking companion to this skill, see planning-with-files.
---
...
**Context Management Setup** — For any plan spanning multiple sessions or >30 minutes of work, create the three tracking files (see `planning-with-files` skill):

```
.plans/<project-name>/
├── task_plan.md   # The plan broken into phases with status
├── progress.md    # Execution log with the 5-Question Reboot Check

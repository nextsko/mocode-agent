...
func TestToPromptXMLBuiltinType(t *testing.T) {
	...
	skills := []*Skill{
		{Name: "builtin-skill", Description: "A builtin.", SkillFilePath: "mocode://skills/builtin-skill/SKILL.md", Builtin: true},
...
func TestDiscoverBuiltin(t *testing.T) {
	...
	for _, s := range discovered {
		if s.Name == "mocode-config" {
			found = true
			require.True(t, strings.HasPrefix(s.SkillFilePath, BuiltinPrefix))
			require.True(t, strings.HasPrefix(s.Path, BuiltinPrefix))
			require.Equal(t, "mocode://skills/mocode-config/SKILL.md", s.SkillFilePath)
			require.Equal(t, "mocode://skills/mocode-config", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	}
	require.True(t, found, "mocode-config builtin skill not found")

	var foundJQ bool
	for _, s := range discovered {
		if s.Name == "jq" {
			foundJQ = true
			require.Equal(t, "mocode://skills/jq/SKILL.md", s.SkillFilePath)
			require.Equal(t, "mocode://skills/jq", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
	...
	var foundHooks bool
	for _, s := range discovered {
		if s.Name == "mocode-hooks" {
			foundHooks = true
			require.Equal(t, "mocode://skills/mocode-hooks/SKILL.md", s.SkillFilePath)
			require.Equal(t, "mocode://skills/mocode-hooks", s.Path)
			require.NotEmpty(t, s.Description)
			require.NotEmpty(t, s.Instructions)
			require.True(t, s.Builtin)
		}
...
func TestDeduplicate(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			input:    []*Skill{{Name: "mocode-config", Path: "mocode://skills/mocode-config"}, {Name: "mocode-config", Path: "/user/mocode-config"}},
			wantLen:  1,
			wantName: "mocode-config",
			wantPath: "/user/mocode-config",
		},
		{
			name:    "empty",
			input:   nil,

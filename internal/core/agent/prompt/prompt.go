...
type Prompt struct {
	...
	platform       string
	workingDir     string
	raw            bool   // if true, skip template parsing and return content as-is
	sessionContext string // branch/snapshot info about current session
}
...
// WithRaw returns an Option that makes the Prompt skip template parsing.
// Use this for raw content that should be passed through as-is without
// Go template interpretation (e.g., custom agent prompts from .md files).
func WithRaw() Option {
	return func(p *Prompt) {
		p.raw = true
	}
...
// WithSessionContext returns an Option that sets branch/snapshot context.
func WithSessionContext(ctx string) Option {
	return func(p *Prompt) {
		p.sessionContext = ctx
	}
}
...
func (p *Prompt) Build(ctx context.Context, provider, model string, store *config.ConfigStore) (string, error) {
	// Raw mode: skip template parsing entirely, return content as-is.
	if p.raw {

...
func (s *Manager) startServer(ctx context.Context, name, filepath string, server *powernapconfig.ServerConfig) {
	...
	if _, err := client.Initialize(initCtx, s.cfg.WorkingDir()); err != nil {
		...
		// Mark as unavailable to avoid retry loops
		// Check if it's a permanent failure (e.g., executable not found)
		errStr := err.Error()
...
// Generally it doesn't matter when shutting down Mocode, though.

...
func NewReferencesTool(lspManager *lsp.Manager) fantasy.AgentTool {
	return fantasy.NewAgentTool(
		...
		func(ctx context.Context, params ReferencesParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			...
			matches, _, err := grep.SearchFiles(ctx, regexp.QuoteMeta(params.Symbol), workingDir, "", 100)
			if err != nil {
...
func find(ctx context.Context, lspManager *lsp.Manager, symbol string, match grep.GrepMatch) ([]protocol.Location, error) {

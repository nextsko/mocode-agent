...
func TestCoderAgent(t *testing.T) {
	...
	if os.Getenv("MOCODE_HYPER_API_KEY") == "" {
		t.Skip("skipping TestCoderAgent: MOCODE_HYPER_API_KEY not set")
	}
	...
	for _, pair := range modelPairs {
		t.Run(pair.name, func(t *testing.T) {
			...
			t.Run("update a file", func(t *testing.T) {
				...
				res, err := agent.Run(t.Context(), SessionAgentCall{
					Prompt:          "update the main.go file by changing the print to say hello from Mocode",
					SessionID:       session.ID,
					MaxOutputTokens: 10000,
				})
				require.NoError(t, err)
				...
				require.NoError(t, err)
				require.Contains(t, strings.ToLower(string(content)), "hello from Mocode")
			})
			...
			t.Run("multiedit tool", func(t *testing.T) {
				...
				res, err := agent.Run(t.Context(), SessionAgentCall{
					Prompt:          "use multiedit to change 'Hello, World!' to 'Hello, Mocode!' and add a comment '// Greeting' above the fmt.Println line in main.go",
					SessionID:       session.ID,
					MaxOutputTokens: 10000,
				})
				require.NoError(t, err)
				...
				content, err := os.ReadFile(mainGoPath)
				require.NoError(t, err)
				require.Contains(t, string(content), "Hello, Mocode!", "Expected file to contain 'Hello, Mocode!'")
			})
...
func TestPreparePrompt_OrphanedToolUse(t *testing.T) {
	...
	// Create an assistant message with a tool call but no tool result 鈥?	// this simulates a cancelled/interrupted agent tool call.
	_, err = env.messages.Create(ctx, sess.ID, message.CreateMessageParams{
...
func TestPreparePrompt_OrphanedToolUseMixed(t *testing.T) {
	...
	// Only one tool result 鈥?for call_ok.
	_, err = env.messages.Create(ctx, sess.ID, message.CreateMessageParams{
		Role: message.Tool,
		Parts: []message.ContentPart{
			message.ToolResult{

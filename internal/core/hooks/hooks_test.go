...
// execute POSIX shell snippets (e.g. echo with single-quoted JSON, exit
// codes, redirections). On Windows the runner falls back to cmd /c which
// does not understand these constructs, so tests authored against sh are
// skipped instead of asserted.
func requirePOSIXShell(t *testing.T) {
	t.Helper()
	if runtime.GOOS == "windows" {
		if _, err := exec.LookPath("sh"); err != nil {
			t.Skip("requires POSIX shell; sh not found on Windows")
		}
	}
}
...
func TestBuildEnv(t *testing.T) {
	...
	require.Equal(t, EventPreToolUse, envMap["MOCODE_EVENT"])
	require.Equal(t, "bash", envMap["MOCODE_TOOL_NAME"])
	require.Equal(t, "sess-1", envMap["MOCODE_SESSION_ID"])
	require.Equal(t, "/work", envMap["MOCODE_CWD"])
	require.Equal(t, "/project", envMap["MOCODE_PROJECT_DIR"])
	require.Equal(t, "ls", envMap["MOCODE_TOOL_INPUT_COMMAND"])
	require.Equal(t, "/tmp/f.txt", envMap["MOCODE_TOOL_INPUT_FILE_PATH"])
}
...
func TestRunnerEnvVarsPropagated(t *testing.T) {
	...
	t.Parallel()
	hookCfg := config.HookConfig{
		Command: `printf '{"decision":"allow","context":"%s"}' "$MOCODE_TOOL_NAME"`,
	}
	r := NewRunner([]config.HookConfig{hookCfg}, t.TempDir(), t.TempDir())
	result, err := r.Run(context.Background(), EventPreToolUse, "sess", "bash", `{}`)
	require.NoError(t, err)
...
func TestAggregationUpdatedInput(t *testing.T) {
	...
	t.Run("null updated_input is a no-op", func(t *testing.T) {
		...
		// never sees a patch 閳?the merged input is empty and the
...
func TestParseStdoutClaudeCodeFormat(t *testing.T) {
	...
	t.Run("Mocode format still works", func(t *testing.T) {
		t.Parallel()

...
// shellCmd returns the shell command and argument for the current platform.
// On Windows we prefer sh.exe if it is on PATH (e.g. shipped with Git for
// Windows or WSL), since hooks are almost always authored as POSIX scripts.
// We fall back to cmd /c only when no POSIX shell is available so the
// runner never silently breaks on a vanilla Windows host.
...
func (r *Runner) runOne(parentCtx context.Context, hook config.HookConfig, envVars []string, payload []byte) HookResult {
	...
	shell, shellArgs := shellCmd(hook.Command)

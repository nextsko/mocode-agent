...
func NewMultiEditTool(
	...
	return fantasy.NewAgentTool(
		...
		func(ctx context.Context, params MultiEditParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
			...
			params.FilePath = ext.SmartJoin(workingDir, params.FilePath)
			...
			editCtx := multiEditContext{ctx, permissions, files, filetracker, workingDir}
			...
			lsp.NotifyLSPs(ctx, lspManager, params.FilePath)

			// Wait for LSP diagnostics and add them to the response
			text := fmt.Sprintf("<result>\n%s\n</result>\n", response.Content)
			text += lsp.GetDiagnostics(params.FilePath, lspManager)
...
func processMultiEditWithCreation(edit multiEditContext, params MultiEditParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {
...
func processMultiEditExistingFile(edit multiEditContext, params MultiEditParams, call fantasy.ToolCall) (fantasy.ToolResponse, error) {

...
**新顶级包目标位置**：`internal/tools/`（模块路径 `github.com/package-register/mocode/tools/`），与 `internal/core/agent/tools/` **同模块但不同包**——这是 PR2 期间出现两个 `tools` 导入路径并存的根本原因。
...
| 新文件 | 职责 |
|---|---|
| `internal/tools/doc.go` | 包文档，说明 toolkit 目的与导入政策 |
| `internal/tools/contracts.go` | `Tool`、`ToolProvider`、`ToolContext`、`ToolResult`、`Schema`、`Capability`、`PermissionChecker`、`MCPHandles`、`ToolCallbacks`、`Attachment` |
| `internal/tools/registry.go` | `Registry` 结构 + `NewRegistry`、`Register`、`RegisterProvider`、`Get`、`Names`、`All` |
| `internal/tools/loader.go` | init() 注册模式 + 重复名 `slog.Warn` |
| `internal/tools/filter/filter.go` | 重写后的过滤器（基于 `tools.Tool`，非 `fantasy.AgentTool`） |
| `internal/tools/filter/filter_test.go` | 迁移并扩展现有测试 |
| `internal/tools/registry_test.go` | 注册表 + loader 测试 |
| `internal/tools/builtin/all/all.go` | umbrella 空白导入 10 个 builtin |
| `internal/tools/plugins/all/all.go` | umbrella 空白导入 30+ 插件 |
| `internal/tools/builtin/<name>/<name>.go` | 搬迁的内置工具 |
| `internal/tools/plugins/<name>/<name>.go` | 搬迁的插件 |
| `internal/tools/mcp/{init,prompts,resources,tools}.go` | 搬迁的 MCP 桥 |
| `internal/tools/lsp/{client,handlers,manager,util/...}.go` | 搬迁的 LSP 桥 |
...
```bash
go build -buildvcs=false ./internal/tools/...
go test -race ./internal/tools/...
go list -deps ./internal/tools/... | grep -E "(fantasy|catwalk|internal/core/(agent|config))"   # 必须空
```
...
- 新建 `internal/tools/doc.go`

**验收**：`go doc github.com/package-register/mocode/tools` 可见包说明；`go build ./internal/tools/...` 通过。
...
- 新建 `internal/tools/contracts.go`
...
**验收**：`go build ./internal/tools/...` 通过；`go vet` 不报 unused interface。
...
- 新建 `internal/tools/registry.go`
- 新建 `internal/tools/loader.go`
...
- 新建 `internal/tools/filter/filter.go`
...
**验收**：`go test ./internal/tools/filter/...` 通过（用 1.6 的测试套件迁移过来）。
...
### 1.5 在 `internal/tools/filter/filter_test.go` 移植测试（scaffold 组）

**文件**：

- 新建 `internal/tools/filter/filter_test.go`
...
**验收**：`go test -race ./internal/tools/filter/...` 全部 PASS。
...
- 新建 `internal/tools/registry_test.go`
...
**验收**：`go test -race ./internal/tools/...` 全部 PASS。
...
**验收**：`go build ./internal/core/agent/...` 通过；无任何 `internal/tools` 的运行时调用。
...
- 修改 `Taskfile.yaml`，在 `lint:` 任务前增加 `lint:deps` 任务：
  ...
  ```yaml
  lint:deps:
    desc: Verify tools/ has no LLM-runtime or agent-package imports
    cmds:
      - "! go list -deps ./internal/tools/... | grep -E '(fantasy|catwalk|internal/core/(agent|config))'"
  ```

- 修改 `lint:` 任务使其依赖 `lint:deps`

**验收**：`task lint:deps` 在 PR1 当前代码上 PASS（空输出）；故意向 `tools/contracts.go` 加 `charm.land/fantasy` 导入后 `task lint:deps` FAIL。

**依赖**：1.2、1.3、1.4。

**Commit**：`ci: add tools/ dependency boundary check`
...
go test -race ./internal/tools/... ./internal/core/agent/...
task fmt
task lint:fix
task lint:deps
```

预期：全绿。`internal/core/agent/tools/` 旧路径**未触动**。
...
**目标**：40+ 工具实现搬到 `internal/tools/...`，旧路径通过 re-export 兼容；期间 `go build ./...` 全程绿。
...
- 新建 `internal/tools/plugins/sshcommon/`（从 `internal/core/agent/tools/plugins/sshcommon/` 复制）
- 新建 `internal/tools/plugins/gitopscommon/`
- 新建 `internal/tools/plugins/giteacommon/`
- 新建 `internal/tools/plugins/netcommon/`
- 新建 `internal/tools/plugins/searchcommon/`

**操作**：

- `cp -r internal/core/agent/tools/plugins/sshcommon/* internal/tools/plugins/sshcommon/`
- 对每个 helper，改包名为 `package sshcommon` 等；**保留**所有导出符号
- 在新位置加 `init()` 空白（留作未来注册点，本 PR 不在此处注册）

**验收**：`go build ./internal/tools/plugins/sshcommon/...` 等 5 个 helper 全部 PASS。
...
### 2.2 搬迁 `tools/builtin/bash/`（builtin 组）

**文件**：

- 新建 `internal/tools/builtin/bash/`
- 从 `internal/core/agent/tools/builtin/bash/` 复制 `bash.go` / `safe.go` / `*.tpl` / `*_test.go`
- 改 import 路径：`internal/core/agent/toolutil` → 暂时保留（下一步处理）；`internal/core/permission` → 暂时保留

**验收**：`go test ./internal/tools/builtin/bash/...` PASS。

**依赖**：1.3。

**Commit**：`refactor(tools/builtin): move bash tool to new path`

---

### 2.3 搬迁 `tools/builtin/edit/`、`view/`、`write/`、`ls/`（builtin 组）

**文件**：4 个新目录，各复制自 `internal/core/agent/tools/builtin/<name>/`。

**注意**：`edit` / `view` / `write` 依赖 `internal/core/agent/tools/lsp` 的 `*lsp.Manager`。**先搬迁 lsp（见 2.20）再回到这里**，或在本步暂时保留 `internal/core/agent/tools/lsp` 旧路径作为依赖。

**验收**：4 个目录的测试单独运行全 PASS。

**依赖**：2.2（模式相同，允许并行执行）。

**Commit**：`refactor(tools/builtin): move file tools to new path`

---

### 2.4 搬迁 `tools/builtin/multiedit/`、`read_files/`（builtin 组）

**文件**：2 个新目录。

**验收**：`go test ./internal/tools/builtin/multiedit/... ./internal/tools/builtin/read_files/...` PASS。

**依赖**：2.3 模式一致，可并行。

**Commit**：`refactor(tools/builtin): move multiedit and read_files`

---

### 2.5 搬迁 `tools/builtin/job_input/`、`job_kill/`、`job_output/`（builtin 组）
...
**Commit**：`refactor(tools/builtin): move job_* tools`

---

### 2.6 创建 `tools/builtin/all/all.go`（builtin 组）

**文件**：

- 新建 `internal/tools/builtin/all/all.go`
- 内容：`package all` + 10 个 `_ "github.com/package-register/mocode/tools/builtin/<name>"` 空白导入

**验收**：

- `go build ./internal/tools/builtin/all/...` PASS
- `internal/tools/builtin/all/all_test.go` 断言 `tools.Default().Names()` 包含所有 10 个 builtin 名（用 `require.Contains`）

**依赖**：2.2–2.5。

**Commit**：`feat(tools/builtin/all): add umbrella blank-import for builtins`

---

### 2.7 旧 `internal/core/agent/tools/builtin/` 改为 re-export（builtin 组）

**文件**：

- 修改 `internal/core/agent/tools/builtin/<name>/<name>.go`（10 个文件）
- 把包主体清空，改为：

  ```go
  // Package <name> is a compatibility re-export. New code should import
  // github.com/package-register/mocode/tools/builtin/<name> directly.
  package <name>

  import "github.com/package-register/mocode/tools/builtin/<name>"
...
- `go build ./...` 全绿
- `go test ./internal/core/agent/tools/builtin/...` 旧测试仍 PASS（re-export 透传）

**依赖**：2.6。

**Commit**：`refactor(agent/tools/builtin): re-export from new toolkit path`
...
- 新建 `internal/tools/plugins/think/`
- 新建 `internal/tools/plugins/todos/`

**验收**：两个目录测试 PASS。

**依赖**：2.1。

**Commit**：`refactor(tools/plugins): move think and todos`
...
**Commit**：`refactor(tools/plugins): move search and web tools`
...
**Commit**：`refactor(tools/plugins): move network tools`
...
- 新建 `internal/tools/plugins/ssh_list_hosts/`
- 新建 `internal/tools/plugins/ssh_exec/`
- 新建 `internal/tools/plugins/ssh_download/`
- 新建 `internal/tools/plugins/ssh_upload/`

**注意**：这 4 个 tool 引用 `internal/tools/plugins/sshcommon`（2.1 已搬迁），改 import 路径。

**验收**：4 个目录测试 PASS。

**依赖**：2.1。

**Commit**：`refactor(tools/plugins): move ssh tools to new path`
...
- 新建 `internal/tools/plugins/git_plan_commits/`
- 新建 `internal/tools/plugins/git_execute_commits/`

**依赖**：`gitopscommon`（2.1）。

**验收**：2 个目录测试 PASS。

**Commit**：`refactor(tools/plugins): move git tools`
...
- 新建 `internal/tools/plugins/gitea_issues/`
- 新建 `internal/tools/plugins/gitea_pulls/`
- 新建 `internal/tools/plugins/gitea_notifications/`

**依赖**：`giteacommon`（2.1）。

**验收**：3 个目录测试 PASS。

**Commit**：`refactor(tools/plugins): move gitea tools`

---

### 2.14 搬迁 `sourcegraph/`（plugins 组）

**文件**：新建 `internal/tools/plugins/sourcegraph/`。

**验收**：目录测试 PASS。

**Commit**：`refactor(tools/plugins): move sourcegraph tool`
...
- 新建 `internal/tools/plugins/send_wechat_file/`
- 新建 `internal/tools/plugins/send_wechat_image/`
- 新建 `internal/tools/plugins/screenshot_to_wechat/`
- 新建 `internal/tools/plugins/message_export/`

**注意**：`send_wechat_*` 引用 `internal/integration/wechat` 的 messenger 端口。搬迁后**仍引用旧路径**（PR3 才统一改）；但 `internal/integration/wechat` 不能反过来引用 `internal/tools/...`，这是单向的，合规。

**验收**：4 个目录测试 PASS。

**Commit**：`refactor(tools/plugins): move wechat tools`
...
- 新建 `internal/tools/plugins/session_export/`
- 新建 `internal/tools/plugins/session_search/`
- 新建 `internal/tools/plugins/session_summary/`

**依赖**：依赖 `session` / `message` / `history` 等 domain service。搬迁后**保持 service 引用**（在 `internal/core/agent` 之外可用）。

**验收**：3 个目录测试 PASS。

**Commit**：`refactor(tools/plugins): move session tools`
...
**Commit**：`refactor(tools/plugins): move mocode and lsp_restart tools`
...
**Commit**：`refactor(tools/plugins): move mcp meta tools`

---

### 2.19 创建 `tools/plugins/all/all.go`（plugins 组）

**文件**：

- 新建 `internal/tools/plugins/all/all.go`
- 内容：`package all` + 30+ `_ "github.com/package-register/mocode/tools/plugins/<name>"` 空白导入

**验收**：

- `go build ./internal/tools/plugins/all/...` PASS
- `internal/tools/plugins/all/all_test.go` 断言 `tools.Default().Names()` 包含所有预期 plugin 名

**依赖**：2.8–2.18。

**Commit**：`feat(tools/plugins/all): add umbrella blank-import for plugins`

---

### 2.20 旧 plugin 目录 re-export（plugins 组）

**文件**：对 `internal/core/agent/tools/plugins/<name>/<name>.go`（30+ 文件）做与 2.7 相同的 re-export 处理。

**验收**：

- `go build ./...` 全绿
- `go test ./internal/core/agent/tools/plugins/...` 旧测试仍 PASS

**依赖**：2.19。

**Commit**：`refactor(agent/tools/plugins): re-export from new toolkit path`

---

### 2.21 搬迁 `tools/mcp/`（mcp-lsp 组）

**文件**：

- 新建 `internal/tools/mcp/`
...
- `go test ./internal/tools/mcp/...` PASS
- `go build ./...` 全绿

**依赖**：1.3。

**Commit**：`refactor(tools/mcp): move MCP bridge to new path with ToolProvider adapter`

---

### 2.22 搬迁 `tools/lsp/`（mcp-lsp 组）

**文件**：

- 新建 `internal/tools/lsp/`
...
**验收**：`go test ./internal/tools/lsp/...` PASS。

**依赖**：1.3。

**Commit**：`refactor(tools/lsp): move LSP bridge to new path`

> **⚠️ 风险见 Risks 6.1**：`internal/tools/lsp/` 当前会因 LSP 实现需要而引入 `charm.land/fantasy`。**`task lint:deps` 必失败**。本任务的实施者必须**显式**把 `lsp` 加入 `lint:deps` 的 grep 例外列表（用 `internal/tools/lsp` 子路径单独 grep），并在 PR2 末尾用 `internal/core/agent/agenttool_adapter.go` 内的薄包装代替 `lsp` 直接对 `fantasy` 的依赖。本任务的 DoD 包含：在 PR3 收尾时**移出** `lsp` 的 `fantasy` 导入。

---

### 2.23 `internal/core/agent/tools/mcp/`、`lsp/` 改为 re-export（mcp-lsp 组）

**文件**：

- 旧 `internal/core/agent/tools/mcp/{init,prompts,resources,tools}.go` 改为对 `internal/tools/mcp` 的 re-export
- 旧 `internal/core/agent/tools/lsp/{client,handlers,manager,util}.go` 改为对 `internal/tools/lsp` 的 re-export
...
- `internal/core/agent/tools/compat.go` → 仅保留 `transfer.go` / `fetch_types.go` / 根级别名
- `internal/core/agent/tools/tools.go` → re-export 来自 `internal/tools` 的新符号
- `internal/core/agent/tools/registry.go` / `registry_filter.go` → 标记 `// Deprecated: moved to internal/tools/...` 并把 `AllToolNames` / `AllToolDescriptors` / `ToolDeps` 等改为对 `internal/tools` 的薄包装，或保留为独立 facade
...
- 对所有 `internal/core/agent/**/*.go` 跑 `gofmt -r` 或 `sed`：
  - `internal/core/agent/tools/builtin/<n>` → `github.com/package-register/mocode/tools/builtin/<n>`
  - `internal/core/agent/tools/plugins/<n>` → `github.com/package-register/mocode/tools/plugins/<n>`
  - `internal/core/agent/tools/mcp` → `github.com/package-register/mocode/tools/mcp`
  - `internal/core/agent/tools/lsp` → `github.com/package-register/mocode/tools/lsp`
  - `internal/core/agent/tools/filter` → `github.com/package-register/mocode/tools/filter`
- 跑完 `go build -buildvcs=false ./...` 确认全绿
- 跑 `grep -r "internal/core/agent/tools" internal/core/agent/ internal/ui/ internal/store/ internal/integration/` 只剩 `internal/core/agent/tools/*.go` 自身

**验收**：`go build ./...` 全绿；`go test -race ./...` 全绿；`grep -r "internal/core/agent/tools/builtin\|internal/core/agent/tools/plugins\|internal/core/agent/tools/mcp\|internal/core/agent/tools/lsp\|internal/core/agent/tools/filter" --include="*.go" | grep -v "^internal/core/agent/tools/"` 为空（允许 shim 文件本身保留旧路径引用）。

**依赖**：2.24。

**Commit**：`refactor: rewrite imports to new tools/ path`
...
- `internal/core/agent/tools/` 旧路径仍存在（作为 re-export shim）
- `internal/tools/...` 已有 40+ 实现 + umbrella `all`
- `go test ./internal/tools/...` 全绿（每个搬过来的 tool 测试独立 PASS）
...
**目标**：`Coordinator` 改用 `*tools.Registry` 走 `Tool` 接口；`agenttool_adapter.go` 成为唯一 `fantasy` 耦合点；旧 `internal/core/agent/tools/` 删除。
...
go test -race ./...
task lint:deps
grep -r "internal/core/agent/tools" --include="*.go" | grep -v "^internal/core/agent/tools/"   # 必须空
```
...
### 3.7 删除 `internal/core/agent/tools/`（cutover 组）

**文件**：整目录删除：

```bash
rm -rf internal/core/agent/tools/
```
...
**Commit**：`chore: remove legacy internal/core/agent/tools/ directory`
...
- 修改 `internal/tools/lsp/` 内的 `fantasy` 引用，改为接收 `tools.Tool` 形式的 LSP 客户端（由 `internal/core/agent/lsp_bridge.go` 新文件适配 `fantasy` 类型）
- 修改 `Taskfile.yaml` 的 `lint:deps`，移除 `internal/tools/lsp` 的例外

**验收**：

- `go list -deps ./internal/tools/... | grep -E "(fantasy|catwalk|internal/core/(agent|config))"` 全空
- `task lint:deps` PASS

**依赖**：2.22（原 lsp 搬迁）、3.1（adapter 已就位）。

**Commit**：`refactor(tools/lsp): remove direct fantasy dependency`
...
- AGENTS.md 新增章节 "Tool Layout" 说明：
  - 所有 tool 位于 `internal/tools/`
  - 添加新 tool = 新建子目录 + 在 `builtin/all/` 或 `plugins/all/` 加一行空白导入
  - 严禁在 `internal/tools/...` 内 import `charm.land/fantasy` / `charm.land/catwalk` / `internal/core/agent` / `internal/core/config`
...
- `internal/core/agent/tools/` 已不存在
- `internal/tools/` 完整，有 `contracts.go` / `registry.go` / `loader.go` / `filter/` / `builtin/all/` / `plugins/all/` / `mcp/` / `lsp/`
...
- `go list -deps ./internal/tools/... | grep -E "(fantasy|catwalk|internal/core/(agent|config))"` 为空
...
- 每个 tool 的 `*_test.go` 随 `cp -r` 一同搬到 `internal/tools/...`
- 测试断言保持不变，行为不变
- 验收：`go test -race ./internal/tools/builtin/...` / `./internal/tools/plugins/...` / `./internal/tools/mcp/...` / `./internal/tools/lsp/...` 全 PASS
- 失败 → 对比 pre-refactor 状态：同一个 tool 在旧路径也是 PASS 吗？若不是，PR2 引入 bug；若是，旧测试在新路径应自动通过

### 3.10.2 Registry / loader 测试（新增）

- 文件：`internal/tools/registry_test.go`（PR1 1.6）、`internal/tools/filter/filter_test.go`（PR1 1.5）
...
| 4 | `internal/tools/lsp/` 因 LSP 实现需要而引入 `fantasy` | 违反"零依赖"约束，CI 红 | PR2 末 + PR3 3.8 显式处理：把 LSP 对 `fantasy` 的依赖挪到 `internal/core/agent/lsp_bridge.go`，tools/lsp 只暴露 `tools.Tool` 接口 | 2.22、3.8 |
...
- 验证：
  ...
  - `go test -race ./...` PASS
  - 旧 `internal/core/agent/tools/` 恢复，新 `internal/tools/` 也保留（因为 PR1 已独立合入）
- **回滚后**：`internal/tools/builtin/all/` 和 `internal/tools/plugins/all/` 仍可作为"无人 import 的孤儿"存在，无害；后续可在独立清理 PR 中删除

### PR3 回滚

- `git revert <PR3 merge commit>`
- **先做这一步**：把 `internal/core/agent/tools/` 从 git 历史中恢复

  ```bash
  git revert <PR3 merge commit>   # 撤销删除
  git checkout <PR2 HEAD> -- internal/core/agent/tools/
  git add internal/core/agent/tools/
  git commit -m "revert PR3: restore legacy tools/ directory"
  ```
...
go test -race ./internal/tools/...           # PR1
...
grep -r "internal/core/agent/tools" --include="*.go" . \
  | grep -v "^internal/core/agent/tools/"   # PR3 末尾必须空
...
- PR1：1–5 全绿，6 仍可命中（`internal/core/agent/tools/` 未删）
...
5. **`tools/` 独立 `go.mod`**—— 留待有第二个 consumer 出现时

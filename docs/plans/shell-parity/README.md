...
| `internal/core/shellruntime/shell/rg.go` | `rg` 命令（Go 兜底 + `hostRgPath()` 宿主探测） |
| `internal/core/shellruntime/shell/pipeutils_test.go` / `fd_test.go` / `rg_test.go` | 逐命令单测 + 经解释器的端到端测试 |
...
- flag 覆盖：`head -n/-c/-q/-v`、`tail -n/+N/-c`、`wc -l -w -c -m -L`、`tee -a`、
  ...
  `rg -i/-s/-S/-F/-w/-v/-l/-c/-o/-g/-m/-A/-B/-C/--hidden/--no-ignore`。
- **`fd`/`rg` 的特例**：检测到宿主同名二进制则**转发**给它（行为与本地一致），
  否则用内置的 gitignore 感知实现（`fd.go` / `rg.go`，遍历复用 `fsext.ListDirectory`）。
  `rg` 的 Go 兜底**不复用** grep 工具的搜索栈（`tools → shellruntime` 单向依赖，反向会成环）。
- 未实现（按分界线保持路由）：`grep/sed/awk`。
